<script lang="ts">
  ...
  const TITLE = "Typhoon 20";
  const DESCRTIPTION = "A Unique Collection of 3d Super Cars. Built for This Decade.";
  const HEADER_TITLE = "SOLAMBOS";
  const HEADER_LINK = "https://bigdefienergy.com/solambos-nfts/";
...
<main class="h-screen">
  ...
  {:else}
    ...
    {#if HEADER_TITLE}
      <a
        href={HEADER_LINK}
        class="text-teal-500 tracking-widest underline underline-offset-4 decoration-2 font-mono"
        >{HEADER_TITLE}</a
      >
    {/if}
    <!-- Card -->
    <div
      class=" max-w-lg mx-auto bg-black rounded-lg my-12  border-2"
      transition:fade
    >
      <!-- Top Bar -->
      <Header />
      ...
      <div class="p-6">
        ...
        <div
          class=" text-lg text-white sm:text-2xl font-mono font-bold py-5 tracking-wider"
        >
          {TITLE}
        </div>
        <div class="text-sm sm:text-md font-semibold pb-5 text-yellow-400 ">
          {DESCRTIPTION}
        </div>
        <Button {connection} />

        <div class=" tracking-widest font-bold text-sm pt-3 text-teal-300">
